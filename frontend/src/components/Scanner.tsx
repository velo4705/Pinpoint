...
const Scanner: React.FC<ScannerProps> = ({ onScanResult, onScanStart, onModeChange, onGameChange, isCollapsed }) => {
  const [isScanning, setIsScanning] = useState(false);
  const [selectedImage, setSelectedImage] = useState<string | null>(null);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [mode, setMode] = useState<'real-world' | 'game'>('real-world');
  ...
  if (isCollapsed) {
    ...
    return (
      <div className="w-full modern-panel p-4 px-6 flex items-center justify-between shadow-2xl border-blue-500 bg-blue-600/5 relative overflow-hidden">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <span className="text-[10px] font-black tracking-[0.2em] uppercase text-blue-500 animate-pulse">
              {getStatusText()}
            </span>
  ...
  return (
    <div className="w-full max-w-xl modern-panel flex flex-col max-h-[85vh] transition-all duration-500 overflow-hidden">
      <div className="flex justify-between items-center border-b border-[var(--border)] p-5 md:p-6 flex-shrink-0">
        <div className="flex flex-col">
          <h2 className="text-sm font-black flex items-center gap-2 tracking-[0.2em]">
            <Scan className="w-4 h-4 text-blue-600" />
            IDENTIFIER
          </h2>
          <span className="text-[9px] text-[var(--text-muted)] font-bold uppercase tracking-widest mt-1">Core Engine v5</span>
        </div>
        <div className="flex bg-[var(--bg)] rounded-lg p-1.5 border border-[var(--border)]">
          <button 
            onClick={() => { setMode('real-world'); onModeChange?.('real-world'); }}
            className={`px-4 py-1.5 rounded-md text-[10px] font-black tracking-wider uppercase transition-all ${mode === 'real-world' ? 'bg-blue-600 text-white shadow-sm' : 'text-[var(--text-muted)] hover:text-[var(--text)]'}`}
          >
            Real World
          </button>
          <button 
            onClick={() => { setMode('game'); onModeChange?.('game'); }}
            className={`px-4 py-1.5 rounded-md text-[10px] font-black tracking-wider uppercase transition-all ${mode === 'game' ? 'bg-blue-600 text-white shadow-sm' : 'text-[var(--text-muted)] hover:text-[var(--text)]'}`}
          >
            Gaming
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8 flex flex-col gap-6 md:gap-8 no-scrollbar">
        <div 
          ...
          className={`relative aspect-[16/10] w-full rounded-2xl border flex flex-col items-center justify-center cursor-pointer transition-all duration-500 group flex-shrink-0 ${selectedImage ? 'border-blue-500/30 bg-blue-500/5' : 'border-[var(--border)] hover:border-blue-500/50 hover:bg-[var(--bg)] shadow-sm'}`}
        >
          <input 
            type="file" 
            ref={fileInputRef} 
          ...
          <AnimatePresence mode="wait">
            {selectedImage ? (
              ...
              >
                ...
                {isScanning && (
                  <div className="absolute inset-0 bg-blue-600/20 backdrop-blur-[4px] flex items-center justify-center rounded-xl overflow-hidden">
              ...
              <motion.div 
                ...
                className="flex flex-col items-center gap-4 text-[var(--text-muted)] group-hover:text-blue-600 transition-colors"
              >
                <div className="p-5 rounded-full bg-[var(--bg)] border border-[var(--border)] group-hover:border-blue-500/50 transition-all shadow-sm">
        ...
        {mode === 'game' && (
          <div className="flex flex-col gap-3">
            ...
            <select 
              ...
              className="bg-[var(--bg)] border border-[var(--border)] rounded-xl p-4 text-xs font-black outline-none focus:border-blue-500/50 transition-all cursor-pointer shadow-sm"
            >
        ...
        <button
          ...
          onClick={startScan}
          className={`w-full py-5 rounded-2xl font-black text-[11px] uppercase tracking-[0.4em] flex items-center justify-center gap-4 transition-all ${!selectedFile || isScanning ? 'bg-[var(--border)] text-[var(--text-muted)] cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 text-white shadow-xl shadow-blue-600/20 active:scale-95'}`}
        >
